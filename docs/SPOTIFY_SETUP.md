...
3. Renseignez l'adresse HTTPS de votre installation dans « Website » et ajoutez exactement son URI de retour dans « Redirect URIs », par exemple `https://famille.example.net/api/v1/music/spotify/callback`.
...
La route de retour n'a pas besoin de répondre au moment de l'enregistrement ; elle doit être disponible quand un membre connecte son compte. Chaque administrateur crée sa propre application et ses propres identifiants. Spotify ne fournit pas d'accès Web API à FamilyHub sans cette étape. Voir le [guide officiel de création d'application](https://developer.spotify.com/documentation/web-api/concepts/apps).
...
SPOTIFY_REDIRECT_URI=https://famille.example.net/api/v1/music/spotify/callback
```
...
Le `.env` local du poste de développement et le `.env` du serveur sont **deux fichiers distincts**. Copier le code ou fusionner une pull request ne copie pas les secrets sur le serveur. `.gitignore` et `.dockerignore` excluent les fichiers `.env` privés ; vérifiez qu'ils ne sont jamais ajoutés à Git. Transférez les quatre valeurs Spotify de manière privée vers le `.env` du serveur. Remplacez l'origine locale `http://localhost:3080` par votre domaine HTTPS dans `FAMILYHUB_ORIGIN`.
