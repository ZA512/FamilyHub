...
export async function registerRoutes(app: FastifyInstance, pool: Pool, config: AppConfig) {
  ...
  const dummyPasswordHash = await hash('familyhub-password-verification-placeholder', passwordOptions);
  ...
  app.post('/api/v1/auth/login', { config: { rateLimit: { max: 10, timeWindow: '15 minutes' } } }, async (request, reply) => {
    ...
    const passwordValid = await verify(row?.password_hash ?? dummyPasswordHash, parsed.data.password);
  ...
  app.post('/api/v1/auth/logout', { preHandler: [requireSession, requireCsrf] }, async (request, reply) => {
    if (request.session) {
      await pool.query('UPDATE session SET revoked_at = now() WHERE id = $1', [request.session.sessionId]);
    }
